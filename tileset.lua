...
tile_lookup[20] = 9  -- allsides
tile_lookup[24] = 10 -- 3 sides
tile_lookup[28] = 12 -- 2 far sides
tile_lookup[32] = 11 -- 2 near sides
tile_lookup[36] = 13 -- 1 side
tile_lookup[40] = 14 -- no sides
function displayLevel(level,width,height,offx,offy,bombs,win)
    lowW = #(level[1])
    lowH = #(level)
    local ndeg = math.pi * .5
    ...
    for y = 0, lowH - 1,1 do
        for x = 0, lowW - 1,1 do
            ...
            elseif level[y + 1][x + 1] == 3 and bombs then
                love.graphics.draw(tileset,sprite[23],x * tileW + offx,y * tileH + offy,0,tileW * .125,tileH * .125)
            elseif level[y + 1][x + 1] >= 20 then
                local r = level[y + 1][x + 1] % 4
                local v = tile_lookup[level[y + 1][x + 1] - r]
                local ox = ({1,1})[r] or 0
                local oy = ({0,1,1})[r] or 0
                love.graphics.draw(tileset,sprite[v],(x + ox) * tileW + offx,(y + oy) * tileH + offy,r * ndeg,tileW * .125,tileH * .125)
            else
                love.graphics.draw(tileset,sprite[
                    tile_lookup[level[y + 1][x + 1]]
                ],x * tileW + offx,y * tileH + offy,0,tileW * .125,tileH * .125)
            end
        end -- [x = 0, lowW - 1,1]
    end -- [y = 0, lowH - 1,1]
end